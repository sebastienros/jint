...
- **Constraints bound one entry into the engine, never a host-driven sequence of them.** Every public entry that runs script — `Execute`, `Evaluate`, `Invoke`, `Engine.Call`, the `JsValue.Call` extension helpers — funnels through `Engine.ExecuteWithConstraints` (`Jint/Engine.cs`), which calls `ResetConstraints()` before the callback and again in its `finally` for any entry that is not nested (nesting is `_executionContexts.Count > 1`). So `foreach (var row in rows) predicate.Call(row);` — the single most common embedding shape — hands every element a fresh statement budget, a fresh allocation baseline and, worst, a **freshly armed timeout deadline**. Measured: `MaxStatements(100)` does not stop 1000 host `Call`s, `TimeoutInterval(200ms)` does not fire across 3 s of continuous host-driven execution, and `LimitMemory` never sees more than one call's allocations — while the identical work inside one `Execute` throws in every case. The reset itself is not the mistake: per-run reset is exactly what `Constraint.Reset`'s doc promises and what makes a reused engine usable, and the nested case is handled deliberately (a host callback re-entering the engine from inside a running script does *not* re-arm, or `while (true) hostCallback()` would run forever). What no embedder expects is that a single function call is a **run**. A second, independent mechanism makes it worse: the amortized check cadence lives on the evaluation context (`EvaluationContext._amortizedConstraintCountdown`, reset to `AmortizedConstraintCheckInterval` = 64 in the constructor) and a top-level host call always builds a new context, so a callee shorter than 64 statements never reaches an amortized check *at all*. That is why **cancellation also fails to span host calls**, even though `CancellationConstraint.Reset()` is deliberately a no-op and the cancelled state does survive across entries: a one-statement predicate invoked a thousand times after `cts.Cancel()` never notices, while a callee long enough to cross the interval throws on its first invocation. `Engine.Constraints.Check()` from the host loop does not close the gap either — `TimeConstraint` re-arms its deadline on the way *out* of every run, so a host-side check measures the time since the last call returned. What an embedder must do instead: bound the loop host-side (its own `Stopwatch`/`CancellationToken`, checked between iterations), or move the loop into the script (`rows.forEach(predicate)`) so the whole traversal is one run and one budget. The only in-engine option today is a user-derived `Constraint` whose `Reset()` is a no-op and which stays *exact* (the default `IsAmortizable`, so it is checked on every statement) — which costs the tight-loop lane above, and which silently reverts to missing short calls if it declares itself amortizable. All of it is pinned from the embedder's side in `Jint.Tests.PublicInterface/HostCallLoopConstraintTests.cs`; those tests assert the behaviour as it is, so changing it is a deliberate act that updates them.
