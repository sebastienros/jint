...
- **Ordinary** (overrides at most `GetOwnProperty`) resolves an own read from a **single** probe: the probe proving the name is not shadowed *is* the read. Such a receiver also gets the prototype-method inline cache, so a warm prototype read does not re-probe the host at all.
...
Release probe counts, pinned by `Jint.Tests.PublicInterface/HostObjectProbeCountTests.cs`: ordinary own-hit **1**, ordinary own-**miss 2** (the probe establishing a miss cannot also produce a value, so the read still ends in a `Get` that re-probes before walking the prototype — the one count the derivation did not improve), exotic **1** either way, and **1** for a member-call base like `host.prop.toUpperCase()` in both columns. Debug builds add the verifier's own probes (3 and 4); never quote those as costs.
