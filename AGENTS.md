...
| touch a page — its loop, a navigation, a form, history, cookies, storage, a worker, a budget | [`Jint.Browser/Runtime/AGENTS.md`](Jint.Browser/Runtime/AGENTS.md) | The one thread that owns a page's engine and its DOM, why a navigation is a fetch off the loop and a new engine on it, what a turn is. Cost: a second thread in the DOM. |
