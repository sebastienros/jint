...
Four things about how it is wired are worth knowing before touching it:
...
| `Engine.Advanced.Retire` / `Engine.IsRetired` — terminal lifecycle; the current script and its nested script-execution calls may finish, new imports and later entries are refused, and pending operations are abandoned before disposal | `Jint/Engine.Advanced.cs`, `Jint/Engine.cs`, `Jint.Tests.PublicInterface/EngineRetirementTests.cs` |
