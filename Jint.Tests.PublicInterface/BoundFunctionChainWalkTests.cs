...
/// <summary>
/// A chain of bound functions is a linked list script can make as long as it likes, and two questions the
...
public class BoundFunctionChainWalkTests
{
    ...
    // net10.0, x64). Each link's own "name" is deleted as soon as it is made: on this branch bind writes
    // "bound " + the target's name eagerly, so an undeleted chain would retain a name per link as long as its
    // depth (quadratic, about 600 MB at a depth of 10,000). Without an own name the next bind reads
    // Function.prototype's empty one, which keeps the build linear and leaves the chain itself unchanged.
    private const string BoundChain = "var f = function () {}; for (var i = 0; i < 200000; i++) { f = f.bind(null); delete f.name; }";
    ...
    // proxy chain — which keeps the case about this walk and not about proxy forwarding. The name is deleted for
    // the same reason as above: a bind of a proxy reads the name of the bound function behind it.
    private const string MixedChain = "var f = function () {}; for (var i = 0; i < 100000; i++) { f = new Proxy(f, {}).bind(null); delete f.name; }";
