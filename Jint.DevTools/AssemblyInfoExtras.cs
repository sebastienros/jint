// The same strong-name public key Jint's own grants carry (Jint/AssemblyInfoExtras.cs); this assembly is
// signed with Jint.snk, so a friend has to name it.
//
// Jint.Browser is on the list because the page-level domains it adds derive from the generated dispatch
// bases here, which are internal for the same reason everything else in this package is: the protocol
// surface is not a compatibility contract until a host has something to hold on to. Jint.Tests.Browser is on
// it for the other half of the same fact: what a page target answers is checked against the protocol's own
// manifest, and every type that check reads -- the manifest, a session, a connection -- is internal here.

[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Jint.Tests.DevTools, PublicKey=")]
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Jint.Browser, PublicKey=")]
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Jint.Tests.Browser, PublicKey=")]
