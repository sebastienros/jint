...
| **Console** → a logged function | Expanding a function — in the Console or in the Scope pane — shows a `[[FunctionLocation]]` row whose `app.js:NN` link opens the editor at the declaration. A function with no such row is either a built-in, which has no declaration, or a script function whose source name no registered script claims. |
