...
You are editing the interpreter. `Jint/Runtime/Interpreter/AGENTS.md` carries the engine-affine vs shareable-state contract and the AST `UserData` invariant that makes a shared `Prepared<Script>` safe, why coverage counters cannot live on a handler node, and what a warmed call site — or a warmed handler tree — retains.
