...
internal abstract class JintBinaryExpression : JintExpression
{
    ...
    /// <see cref="InteropHelper.CalculateMethodParameterScore"/>'s gray-zone rule reads it, so two engines
    ...
    /// Only an engine running the stock <see cref="ClrTypeConverter"/> reads or writes this; see
    ...
    /// installed <see cref="ClrTypeConverter"/>, whose answer is the last rule outright — a conversion it
    ...
    /// The converter is an object a host builds — <c>SetTypeConverter</c> hands its factory the
        ...
        [NotNullWhen(true)] out object? result)
    {
        ...
        if (left != null && right != null)
        {
            ...
            var cache = engine._typeConverterTargetFilter is null
                ? _knownOperators
