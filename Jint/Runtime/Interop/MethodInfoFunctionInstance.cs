    ...
    internal sealed class MethodInfoFunctionInstance : FunctionInstance
    {
        ...
        public override JsValue Call(JsValue thisObject, JsValue[] jsArguments)
        {
            ...
            foreach (var (method, arguments, _) in TypeConverter.FindBestMatch(_engine, _methods, ArgumentProvider))
            {
                ...
                {
                    var exception = new System.Exception("JINT is not thread-safe!  This instance of the JINT engine was initialized with a different thread (" + Engine.Thread?.Name + ") than the thread (" + currentThread?.Name + ") being used to call this method (" + method.Method.Name + ")!");
                    throw exception;
