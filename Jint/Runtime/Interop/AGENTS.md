...
- **A wrapper's member-*miss* lane forwards to the prototype and then inspects the answer, so it is a hand-over and it probes the native stack.** The `ThrowOnUnresolvedMember` post-check is what stops the forward being a tail call, so a chain of **adjacent** wrappers - which script builds, `SetPrototypeOf` not being overridden - costs a native frame per link, and twenty thousand of them ended the host process ([#4087](https://github.com/sebastienros/jint/issues/4087)). `ObjectWrapper.Get`'s own comment says why it is the only site of that shape here and why the `Function.Call` IL pin does not cover it.
