...
public abstract class GeneratedReflectionAccessor : ReflectionAccessor
{
    ...
    // box back to object on the cold edge for compatibility with whatever called us. The declared
    // type is the member's own: a generated accessor carries no indexer, so the indexer probe the
    // base class runs -- the only thing that can make valueType differ from MemberType -- cannot
    // apply here.
    public override object? GetValue(Engine engine, object target, string memberName, out Type? valueType)
    {
        valueType = MemberType;
        return GetJsValue(engine, target).ToObject();
    }
