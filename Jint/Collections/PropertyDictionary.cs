...
namespace Jint.Collections
{
    ...
    internal sealed class PropertyDictionary : IReadOnlyCollection<KeyValuePair<JsValue, PropertyDescriptor>>
    {
        ...
        public bool ContainsKey(JsValue key)
        {
        ...
        public bool TryGetValue(JsValue key, out PropertyDescriptor value)
        {
            Entry[] entries = _entries;
            for (int i = _buckets[key.GetHashCode() & (_buckets.Length - 1)] - 1;
                (uint)i < (uint)entries.Length; i = entries[i].next)
       ...
       // Not safe for concurrent _reads_ (at least, if either of them add)
        ...
        public ref PropertyDescriptor GetOrAddValueRef(JsValue key)
        {
            Entry[] entries = _entries;
            int bucketIndex = key.GetHashCode() & (_buckets.Length - 1);
        ...
        public ref PropertyDescriptor this[JsValue key]
        {
        ...
        private ref PropertyDescriptor AddKey(JsValue key, int bucketIndex)
        {
            Entry[] entries = _entries;
            int entryIndex;
            if (_freeList != -1)
        ...
        /// Gets an enumerator over the dictionary
        /// </summary>
        IEnumerator<KeyValuePair<JsValue, PropertyDescriptor>> IEnumerable<KeyValuePair<JsValue, PropertyDescriptor>>.GetEnumerator() =>
            new Enumerator(this);
        ...
        public struct Enumerator : IEnumerator<KeyValuePair<JsValue, PropertyDescriptor>>
        {
            private readonly PropertyDictionary _dictionary;
            private int _index;
            private int _count;
            private KeyValuePair<JsValue, PropertyDescriptor> _current;

            internal Enumerator(PropertyDictionary dictionary)
            {
            ...
            public bool MoveNext()
            {
                ...
                _current = new KeyValuePair<JsValue, PropertyDescriptor>(
                    _dictionary._entries[_index].key,
                    _dictionary._entries[_index++].value);
                return true;
            }

            public KeyValuePair<JsValue, PropertyDescriptor> Current => _current;
