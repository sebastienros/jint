...
{
    /// <summary>
    /// Stores <paramref name="value"/> as an own property named <paramref name="name"/>, replacing any own
    /// property of that name and validating nothing.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The write always lands as an <em>own</em> property on the receiver and therefore <em>shadows</em>
    /// anything of that name inherited from the prototype chain; no inherited setter is invoked. If the
    /// member is — or later becomes — prototype-resident, this silently creates a shadow rather than
    /// updating the inherited member, and a data descriptor can end up shadowing an inherited accessor.
    /// (<c>Error.prototype.stack</c> is a concrete example of a member that lives on the prototype as an
    /// accessor.)
    /// </para>
    /// <para>
    /// No <c>[[DefineOwnProperty]]</c> validation runs — extensibility, the configurable/writable flags of
    /// an existing property, and the data/accessor compatibility rules are all ignored, so the call always
    ...
    /// <summary>
    /// Stores <paramref name="value"/> as an own property keyed by <paramref name="property"/>, replacing
    /// any own property of that name and validating nothing.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The write always lands as an <em>own</em> property on the receiver and therefore <em>shadows</em>
    ...
    /// </para>
    /// </remarks>
    public void DefineOwnPropertyUnchecked(JsValue property, PropertyDescriptor value)
    {
    ...
    /// <paramref name="name"/>, replacing any own property of that name and validating nothing.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The write always lands as an <em>own</em> property on the receiver and therefore <em>shadows</em>
    /// anything of that name inherited from the prototype chain; no inherited setter is invoked. If the
    /// member is — or later becomes — prototype-resident, this silently creates a shadow rather than
    /// updating the inherited member, and the data property written here can end up shadowing an inherited
    /// accessor. (<c>Error.prototype.stack</c> is a concrete example of a member that lives on the prototype
    /// as an accessor.)
    /// </para>
    /// <para>
    /// No <c>[[DefineOwnProperty]]</c> validation runs — extensibility, the configurable/writable flags of
    ...
    /// object gets a descriptor per property and a property dictionary, and the script reading the batch
    /// never keeps a monomorphic inline cache. Use
    /// <see cref="JsObject.Create(Engine, JsObjectLayout, ReadOnlySpan{JsValue})"/> when the property names
    /// are known up front, or
    ...
    public void DefineOwnDataPropertyUnchecked(string name, JsValue value)
    {
