...
#pragma warning restore MA0023
    ...
    public static string FormatDuration(DurationRecord duration)
    {
        ...
        if (absYears != 0) sb.Append($"{absYears}Y");
        if (absMonths != 0) sb.Append($"{absMonths}M");
        if (absWeeks != 0) sb.Append($"{absWeeks}W");
        if (absDays != 0) sb.Append($"{absDays}D");
        ...
        {
            sb.Append('T');
            if (absHours != 0) sb.Append($"{absHours}H");
            if (absMinutes != 0) sb.Append($"{absMinutes}M");
