...
public sealed record HandlerTreeCacheReport
{
    internal HandlerTreeCacheReport(int functionDefinitions, int scriptStatementLists, int evaluatedScripts, int propertyKeyExpressions)
    {
        ...
        PropertyKeyExpressions = propertyKeyExpressions;
    }
    ...
    /// <summary>
    /// Cached handler trees for evaluated property key expressions — computed keys in object literals,
    /// class bodies and destructuring patterns — keyed on the AST node. The stable handler identity is what
    /// lets a suspension inside such a key find its replay state again.
    /// </summary>
    /// <remarks>
    /// Like <see cref="FunctionDefinitions"/> it resets wholesale at 2048 entries, so a reading at or just
    /// above that ceiling is the backstop working, not a leak.
    /// </remarks>
    public int PropertyKeyExpressions { get; }
}
