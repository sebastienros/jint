...
public sealed class JintAdditionalData : IDictionary<string, object?>
{
    ...
    void ICollection<KeyValuePair<string, object?>>.CopyTo(KeyValuePair<string, object?>[] array, int arrayIndex) => ((IDictionary<string, object?>) Options).CopyTo(array, arrayIndex);
    ...
    {
        if (Options.TryGetValue(key, out var val) && val is TValue tValue)
        {
            value = tValue;
            return true;
        }
