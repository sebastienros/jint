...
internal static class WptBrowserCauses
{
    ...
    /// this section counts. If one cause reaches both sets, its row still reports every failure that cause
    /// owns; the surrounding prose must then explain why the rows sum past the six suites' census figure.
    /// </remarks>
    ...
    /// theory knowing it exists. The census retains harness errors separately, and the measured cause check
    /// validates every report before using this list; partial results can therefore never rewrite the table.
    /// </remarks>
