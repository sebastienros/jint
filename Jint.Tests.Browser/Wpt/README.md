...
| 19 | 6 | [#3949](https://github.com/sebastienros/jint/issues/3949) **A tag query cannot recover a null namespace lost on insertion.** AngleSharp 1.8.1 preserves HTML local-name case; the casing exclusions in `case.html` and both tag-query pairs have been removed. A null-namespace `<body>` still becomes an XHTML one on insertion, leaving the two empty-namespace assertions and the HTMLness assertion. The numbers await the authoritative Windows census. <!-- cause: a tag query's namespace and local-name identity --> |
