...
static collection. Their `timeout=long` metadata gives the upstream harness 60 seconds and the driver's
backstop 90 seconds, so a loaded runner does not cut them off first. Nothing in the lane waits on a
