...
Across the six of them there are 222 documents and 40,338 tests, and **1,541 of those tests do not pass**.
...
| 62 | 14 | **One assertion each or one small family per document.** These cover conversion order, import/clone identity, attribute selection and ordering, element-name identity, node equality and `accessKeyLabel`; each pattern is kept separate where neighboring rows pass. <!-- cause: one assertion each --> |
