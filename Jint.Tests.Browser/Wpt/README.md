...
Across the six of them there are 219 documents and 29,003 tests, and **1,118 of those tests do not pass**.
...
| 122 | 3 | **The Selectors-API table and selector-only element states.** The three newly vendored documents cover selector-error contracts, link state, enabled links, no-namespace selectors and `::slotted`; all 122 rows are `NeedsTriage`. <!-- cause: the Selectors-API table and selector-only element states --> |
...
| 38 | 1 | **AngleSharp has no `HTMLDListElement`.** `reflection-grouping.html` can therefore expose no `compact` IDL member on `<dl>` without incorrectly placing it on every `HTMLElement`. <!-- cause: 4. AngleSharp has no HTMLDListElement --> |
...
**3,313 tests, of which 3,191 pass** — and `dom/nodes/` grows from 4,802 tests to 8,115. The 122 that do
