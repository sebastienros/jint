...
| ---: | ---: | --- |
| 137 | 1 | **ProcessingInstruction parsing and element serialization remain.** The attribute map is implemented; 20 rows need HTML PI parsing ([#4098](https://github.com/sebastienros/jint/issues/4098)) and four compare against the element serializer ([#4109](https://github.com/sebastienros/jint/issues/4109)). <!-- cause: a member of a DOM interface the bindings do not have --> |
...
already exist and their attribute map is implemented. The fix removed 96 failures; 20 still need HTML PI parsing and four need element serialization escaping. Another 17 assert that an ill-formed PI-only XML document preserves a PI before its error tree, contrary to HTML §8.5.1; these are `AssertsWhatNothingRequires`, not engine debt. **42 were the name refusals the table already named**, reached three times each; [#3950](https://github.com/sebastienros/jint/issues/3950) made them pass and took the rows out. **The rest are
...
**3,313 tests, of which 3,297 pass** — and `dom/nodes/` grows from 4,802 tests to 8,115. The 16 that do
not pass are bounded to the patterns in the exclusion table, and all 16 are in the third document: the
...
the `::slotted()` functional pseudo-element, whose native parser dispatch is a private frozen dictionary
with no registration point. The empty namespace prefix now uses `DomSelectors`: a token adapter and a
public predicate factory composed with the native parser and matcher. They are `NeedsTriage`, for the reason that category
