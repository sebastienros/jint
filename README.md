...
| `Network`'s WebSocket events — a socket's handshake is not a fetch, so the engine does not observe it — and its `eventSourceMessageReceived` event, since what an observer is handed is the stream's bytes rather than the events they decode into. Its **timing** document is published, but only for the interval the transport can see exactly: `requestTime` and `receiveHeaders*` are real, and the phases behind the host's own `HttpClient` — proxy, DNS, connect, TLS, worker, push — are `-1` rather than a zero that would read as a phase which happened instantly | [#3701](https://github.com/sebastienros/jint/issues/3701) |
