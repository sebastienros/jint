...
namespace Jint.Tests.Runtime
{
    public class EngineTests : IDisposable
    {
        ...
        public void ShouldNotInvokeNonFunctionValue()
        {
            ...
            var exception = Assert.Throws<JintScriptExecutionException>(() => x.Invoke(_engine, 1, 2));
        ...
        public void ShouldNotInvokeNonFunctionValueThatBelongsToAnObject()
        {
            ...
            Assert.Throws<JintScriptExecutionException>(() => _engine.Invoke(foo, obj, new object[] { }));
        }
        ...
        {
            var ex = Assert.Throws<JintScriptExecutionException>(() => _engine.Evaluate("JSON.parse('');"));
...
var prep = function (fn) { fn(); };
        ...
        public void ShouldNotAllowDuplicateProtoProperty()
        {
            ...
            ex = Assert.Throws<JintScriptExecutionException>(() => _engine.Execute($"eval('{code}')"));
            Assert.Contains("Duplicate __proto__ fields are not allowed in object literals", ex.Message);

            Assert.Throws<JintScriptExecutionException>(() => _engine.Execute($"new Function('{code}')"));
            Assert.Contains("Duplicate __proto__ fields are not allowed in object literals", ex.Message);
        }
...
;
        ...
        public void ExceptionShouldHaveLocationOfInnerFunction()
        {
            ...
            var ex = Assert.Throws<JintScriptExecutionException>(() => engine.Evaluate(source));
        ...
        {
            var engine = new Engine();
            var ex = Assert.Throws<JintScriptExecutionException>(() => engine.Execute(source));
...
";
        ...
        {
            var engine = new Engine();
            var ex = Assert.Throws<JintScriptExecutionException>(() => engine.Evaluate("JSON.parse('[01]')"));
...
x.test = {
    ...
    init (a, b) {
        ...
        public void ShouldOverrideDefaultTypeConverter()
        {
            ...
            Assert.Throws<JintScriptExecutionException>(() => engine.Evaluate("c.Name"));
        }
        ...
        {
            var ex = Assert.Throws<JintScriptExecutionException>(() => _engine.Evaluate("typeof MyClass; class MyClass {}"));
        ...
        public void ShouldObeyScriptLevelStrictModeInFunctions()
        {
            ...
            ex = Assert.Throws<JintScriptExecutionException>(() => engine.Evaluate(source2));
        ...
        public void ShouldSupportThisInSubclass()
        {
            ...
            var ex = Assert.Throws<JintScriptExecutionException>(() => engine.Evaluate(script));
