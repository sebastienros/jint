...
public class WhiteSpaceDefinitionTests
{
    ...
    [Theory]
    [InlineData("(nel + 'abc').trim() === nel + 'abc'")]
    [InlineData("(nel + 'abc').trimStart() === nel + 'abc'")]
    [InlineData("('abc' + nel).trimEnd() === 'abc' + nel")]
    [InlineData("(nel + 'abc' + nel).trim() === nel + 'abc' + nel")]
    [InlineData("isNaN(parseInt(nel + '12'))")]
    [InlineData("isNaN(parseFloat(nel + '1.5'))")]
    [InlineData("isNaN(Number(nel + '12'))")]
    [InlineData("isNaN(Number('12' + nel))")]
    [InlineData("isNaN(Number(nel))")]
    [InlineData("isNaN(+(nel + '12'))")]
    [InlineData("!/\\s/.test(nel)")]
    ...
    [Theory]
    [InlineData("(bom + 'abc').trim() === 'abc'")]
    [InlineData("('abc' + bom).trimEnd() === 'abc'")]
    [InlineData("parseInt(bom + '12') === 12")]
    [InlineData("parseFloat(bom + '1.5') === 1.5")]
    [InlineData("Number(bom + '12') === 12")]
    [InlineData("Number('12' + bom) === 12")]
    [InlineData("Number(bom) === 0")]
    [InlineData("/\\s/.test(bom)")]
    ...
    /// </summary>
    [Theory]
    [InlineData("(c + 'x' + c).trim() === 'x'", "", TestDisplayName = "String.prototype.trim")]
    [InlineData("(c + 'x').trimStart() === 'x'", "", TestDisplayName = "String.prototype.trimStart")]
    [InlineData("('x' + c).trimEnd() === 'x'", "", TestDisplayName = "String.prototype.trimEnd")]
    // "+12" and "012" carry the StrDecimalLiteral's own leading sign and leading zero.
    [InlineData("parseInt(c + '12') === 12", "002B,0030", TestDisplayName = "parseInt")]
    [InlineData("parseFloat(c + '1.5') === 1.5", "002B,0030", TestDisplayName = "parseFloat")]
    [InlineData("Number(c + '12') === 12", "002B,0030", TestDisplayName = "Number with a leading pad")]
