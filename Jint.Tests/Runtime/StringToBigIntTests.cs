...
public class StringToBigIntTests
{
    ...
    /// </summary>
    [Theory]
    [InlineData("BigInt('+12')", "12")]
    [InlineData("BigInt('-12')", "-12")]
    [InlineData("BigInt('12')", "12")]
    [InlineData("BigInt('+0')", "0")]
    [InlineData("BigInt('-0')", "0")]
    [InlineData("BigInt('+007')", "7")]
    [InlineData("BigInt('+9007199254740993')", "9007199254740993")]
    ...
    /// <summary>
    /// <c>StringIntegerLiteral</c> is white-space-padded on both sides, and the sign sits inside the padding
    /// rather than outside it.
    /// </summary>
    /// <remarks>
    /// U+FEFF is deliberately absent: this branch trims with <c>string.Trim()</c>, which is the framework's
    /// white-space set rather than the grammar's, so a zero-width no-break space is not padding here. That
    /// divergence is its own defect and not about the sign.
    /// </remarks>
    [Theory]
    [InlineData("' +12 '")]
    [InlineData("String.fromCharCode(0x09) + '+12' + String.fromCharCode(0x0A)")]
    [InlineData("String.fromCharCode(0x00A0) + '+12' + String.fromCharCode(0x2028)")]
    ...
    /// </summary>
    [Theory]
    [InlineData("12n == '+12'", true)]
    [InlineData("'+12' == 12n", true)]
    [InlineData("-12n == '-12'", true)]
    [InlineData("12n == '+13'", false)]
    [InlineData("12n == '+0x0c'", false)]
    [InlineData("0n == '+0'", true)]
    ...
    /// </summary>
    [Theory]
    [InlineData("12n < '+13'", true)]
    [InlineData("12n < '+11'", false)]
    [InlineData("'+11' < 12n", true)]
    [InlineData("'+13' < 12n", false)]
    [InlineData("12n < '+0x1f'", false)]
