...
";
    ...
    /// <summary>
    /// Serves modules from a dictionary keyed by absolute url, resolving relative specifiers against the
    /// importing module the way a browser does - which is only possible if that module knows its own url.
    /// </summary>
    ...
    /// <summary>
    /// Reports each module's location through <c>import.meta.url</c>, which is the only way a module's
    /// own name is observable from script - Jint leaves import.meta to the host.
    /// </summary>
    private sealed class ImportMetaUrlHost : Jint.Runtime.Host
    {
        public override List<KeyValuePair<JsValue, JsValue>> GetImportMetaProperties(Module module)
        {
            var properties = base.GetImportMetaProperties(module);
            properties.Add(new KeyValuePair<JsValue, JsValue>("url", module.Location ?? JsValue.Undefined));
            return properties;
        }
    }

    [Fact]
    public void LoadedModuleKnowsItsWholeUrl()
    {
        var engine = new Engine(o => {
            o.EnableModules(new UrlModuleLoader(new Dictionary<string, string>
            {
                ["http://localhost/lib/entry.js"] = "export const url = import.meta.url;",
            }));
            o.UseHostFactory(_ => new ImportMetaUrlHost());
        });

        var ns = engine.Modules.Import("http://localhost/lib/entry.js");

        ns.Get("url").AsString().Should().Be("http://localhost/lib/entry.js");
    }

    [Fact]
    public void LoadedModuleResolvesARelativeImportAgainstItsOwnUrl()
    {
        // The referrer a module's own imports are resolved against is its location, so reducing that
        // to a path leaves './dep.js' nothing to resolve against.
        var engine = new Engine(o => o.EnableModules(new UrlModuleLoader(new Dictionary<string, string>
        {
    ...
    {
        // A file url is still reported as a filesystem path, which is what every existing host sees.
        var path = Path.Combine(Path.GetTempPath(), "jint-location-test");
        ...
        {
            var engine = new Engine(o => {
                o.EnableModules(path);
                o.UseHostFactory(_ => new ImportMetaUrlHost());
            });
            var ns = engine.Modules.Import("./entry.js");

            var url = ns.Get("url").AsString();
