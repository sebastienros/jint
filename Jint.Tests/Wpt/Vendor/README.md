...
| `html/dom/` | 11 `.html`, 9 `.js` | ARIA reflection, `accessKeyLabel`, HTML's historical members, and `reflection-misc.html`, `-text.html`, `-grouping.html`, `-metadata.html`, `-sections.html` and `-obsolete.html` with the helpers they load — six of HTML's ten reflection documents. The other four are the browser lane's README |
...
<!-- generated by WptCensus.RenderDrift: the drift recipe, and the corpus it walks -->
...
Silence is a clean corpus, and at this pin there are 894 files in 79 directories to be silent
about — 364 of them the documents the browser lane navigates to, the rest the scripts, payloads and
sidecars every lane reads.

<!-- end generated -->

Run over every one of them at this pin: **no drift, and no file absent upstream**. Four files sit outside that
...
file extensions where the corpus vendors eight, so the `.asis` responses and the `.headers` sidecars had never
been compared with upstream at all. Both are read off the corpus now, which means a bump moves them — and the
sentence you are reading is then about a smaller tree and has to be earned again.
