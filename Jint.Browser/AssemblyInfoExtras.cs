// The same strong-name public key Jint's own grants carry (Jint/AssemblyInfoExtras.cs); this assembly is
// signed with Jint.snk, so a friend has to name it.
//
// Everything in this package is internal: the binding layer is a working surface, not a compatibility
// contract, until the runtime that consumes it (campaign phase 2) has settled what a host actually holds.
// The test project is therefore the only consumer, and it has to be named here to see any of it.

[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Jint.Tests.Browser, PublicKey=")]
