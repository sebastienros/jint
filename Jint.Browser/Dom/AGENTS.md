...
| `mergedInterfaces` | An interface **the standard merged into another one**, projected as that other one. Today: `ISettableTokenList`, which still carries `[DomName("DOMSettableTokenList")]` — a name DOM removed in 2016 — so `iframe.sandbox`, `output.htmlFor` and `link.sizes` answered `[object DOMSettableTokenList]`, were not `instanceof DOMTokenList`, and the removed interface was a global. Unlike `excludedInterfaces`, the members whose CLR type names it keep their projection: `LookupInterface` answers with the target's model, so only the definition, the interface object and the type-map candidate go. The merged interface's own members go with it, which is what a merge means — declare any that must survive on the target. |
| `unscopables` | WebIDL's `[Unscopable]`, which is an extended attribute on the IDL declaration and therefore nothing a CLR annotation can carry. DOM marks eight members — `ParentNode`'s `append`/`prepend`/`replaceChildren`, `ChildNode`'s `before`/`after`/`replaceWith`/`remove`, and `Element`'s `slot` — and the list is per **projected** interface, because the two mixins are not interfaces this binding emits. It matters more than `with` does: an [event handler content attribute](https://html.spec.whatwg.org/multipage/webappapis.html#getting-the-current-value-of-the-event-handler)'s scope chain is object environments carrying the *withEnvironment* flag, so without it `<div onclick="remove()">` called the element's `remove` where the standard resolves the global's. Every name is checked against a member the interface really declares — an inherited one would put the key on the wrong prototype. |
