...
**One model answers every geometric question, and that is the point.** `Element.getBoundingClientRect`,
`document.elementFromPoint`, `DOM.getBoxModel`, `DOM.getContentQuads`, `DOM.getNodeForLocation` and
`Input.dispatchMouseEvent(x, y)` are all `Layout/FlatLayout` — every rendered element gets an ordinal in
tree order and owns one row of the page — so a client that reads a box, clicks its centre and asks what was hit is
...
[`../Layout/box-model.md`](../Layout/box-model.md), beside the classes themselves.**
