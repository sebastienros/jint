...
### A row through `Page.EvaluateAsync` must amortise the mailbox round trip

Every `Jint.Browser` benchmark measures a `Page` call rather than a bare `Engine.Evaluate`, and `Page.EvaluateAsync` is a mailbox request: it crosses to the page loop thread and back, which is a real, additive per-invocation cost with its own scheduling jitter. **A row whose script runs its query once measures that round trip, not the query**, whenever the query itself is cheap — and the round trip does not cancel against a baseline row the way a multiplicative cost (clock speed, thermal state) does, because it is additive and independently noisy per invocation.

This is not a theoretical risk. `BrowserSelectorBenchmark`'s first cut ran each query once per `[Benchmark]` invocation, and a paired run against unrelated code — three of its rows (`Control`, `DefaultButton`, `Valid`) whose cost that specific change could not touch at all — swung +4.7%, +19.5% and +18.6% between two builds with identical selector code, purely from round-trip jitter. The two rows whose single pass already cost a millisecond or more (`TargetMissing`, `Indeterminate`) were fine (−99.2% and +1.5%, the first a real 123× improvement and the second real noise-floor agreement): the round trip was a rounding error against real work, so it told the truth.

**The fix is the one `BrowserNodeListBenchmark` already uses** (`Passes = "100"`; `BrowserHtmlCollectionBenchmark` and `BrowserSelectorBenchmark` size theirs per row): loop the query inside the script and sum a value that depends on every pass, so the round trip is amortised over many passes and nothing in the loop is optimised away. **Size the pass count per row, not once for the whole class** — a class can easily contain rows whose single-pass cost differs by three orders of magnitude (a cheap selector over a small subtree versus one that walks the whole document), and one shared pass count either leaves the cheap rows round-trip-dominated or makes the expensive ones absurdly slow. Say what each row's number was chosen for in the doc comment — "already above the round trip, needs no loop" or "cheap enough that it must be looped" — so the next person does not "simplify" two deliberately different constants into one.
