...
///
/// <para>
/// On this branch the member-read inline cache on <c>JintMemberExpression</c> remembers a holder only when it
/// is the receiver's <em>direct</em> prototype; anything deeper re-walks the chain and pays a
/// <c>GetOwnProperty</c> at every level, on every read (<c>main</c> lifted that in sebastienros/jint#4048,
...
/// This class is the engine-only statement of that cost, so the walk is measurable without a host object, a
/// wrapper or a binding generator in the picture. The rows are <c>main</c>'s, unchanged, so the two
/// branches' tables read side by side.
/// </para>
///
...
/// up. <b>The comparison, and the second control:</b> this is the shape the member cache serves, so this row
/// moving is a regression.
/// </description></item>
/// <item><description>
/// <see cref="RootPrototypeGetterRead"/> — the same getter, declared on the <em>root</em> of a four-level
/// hierarchy and read off a leaf instance, so it sits three links behind the holder. On this branch the
/// member cache does not serve it, so it is resolved by <c>ObjectInstance.Get</c>'s prototype walk on every
/// read.
/// </description></item>
/// <item><description>
...
/// each prototype (sebastienros/jint#4076), and nothing absent can be cached on any branch. It reads
/// <c>NaN</c> into the accumulator, which costs the same add as every other row.
/// </description></item>
/// <item><description>
...
/// cache fields, so the pair is what shows the call lane behaves as the read lane does.
/// </description></item>
/// </list>
///
/// <para>
...
public class PrototypeChainReadBenchmark
{
    ...
    /// <summary>The comparison: a getter one link up, the shape the member cache serves.</summary>
    [Benchmark]
    public JsValue DirectPrototypeGetterRead() => _directGetterRead.Run();

    /// <summary>The same getter, four links up: a prototype walk on every read on this branch.</summary>
    [Benchmark]
